...
@pytest.mark.parametrize("title, content, published", [
    ("mountain", "himalaya", True),
    ("river", "ganga", True),
    ("forest", "sundarvan", False)
])
...
def test_create_post_default_published_true(authorized_client, test_user):
    res = authorized_client.post("/posts/", json = {
        "title": "bear", "content": "polar bear", "published": True})

    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "bear"
    assert created_post.content == "polar bear"
...
def test_update_post_not_exist(authorized_client, test_posts):
    data = {"title": "Updated title", "content": "Updated content", "id": test_posts[3].id}
    res = authorized_client.put("/posts/111", json = data)
    assert res.status_code == 404